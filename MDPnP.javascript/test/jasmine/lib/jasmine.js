...
jasmine.ActionCollection.prototype.getTimeElapsed = function(){
	if(this.afterTime) {
		var ms = this.afterTime.getTime() - this.beforeTime.getTime();
		return ms/1000;
	} else {
		return -1;
	}
...
/**
 * The most recent time it took this spec to run
 * @return the time in seconds that it took this spec to run, -1 if the spec has not been run yet
 */
jasmine.Spec.prototype.getTimeElapsed = function() {
	if(this.afterTime) {
		var ms = this.afterTime.getTime() - this.beforeTime.getTime();
		return ms/1000;
	} else {
